...
`registry.json` is the zero-gap index for the target package. It registers every canonical `surface.state` at 1280 × 720 and each surface's responsive-risk state at the four approved smaller effective sizes. Every PNG has a same-name YAML accessibility annotation under `targets/<surface>/` and is generated by the contract-pinned review renderer under `renderer/`.

The renderer and targets are review evidence, not production UI. Their embedded provenance label and registry SHA-256 hashes prevent an unlabelled or competing image from being mistaken for the approved target. WinUI actuals and diffs remain separate downstream evidence.
