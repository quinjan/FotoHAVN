...
function getEventName() {
  return new URLSearchParams(window.location.search).get("event") || "Mika & Paolo's Wedding";
}
...
function StartUnavailableScreen({ kind, onRetry, onExit, eventName }) {
  ...
  return (
    <main className="start-unavailable-screen">
      ...
      <section role="alert">
        ...
        <p className="event-label">{eventName}</p>
...
function PhotoStrip({ eventName }) {
  return (
    <figure className="photo-strip" aria-label="Your four-Capture Photo Strip">
      ...
      <figcaption>{eventName}</figcaption>
    </figure>
  );
}

function StartScreen({ variant, onStart, onExit, eventName }) {
  if (variant === "B") {
    return (
...
function ExitConfirmation({ onCancel, onConfirm, eventName }) {
  return (
    <div className="dialog-layer">
      <section className="confirm-dialog" role="alertdialog" aria-modal="true" aria-labelledby="exit-title" aria-describedby="exit-copy">
        <h2 id="exit-title">Exit “{eventName}”?</h2>
...
function ExitedEventScreen({ eventName }) {
  return (
    <main className="exited-screen">
      <header><Brand /><span>Operator console</span></header>
      <section><span className="complete-mark"><Check weight="bold" /></span><p className="event-label">Saved Events</p><h1>Event exited.</h1><p>“{eventName}” is inactive. The production UI returns to the Saved Events landing page here.</p></section>
    </main>
  );
}
...
function StripScreen({ variant, remaining, leaving, eventName }) {
  return (
    <main className={`strip-screen strip-${variant.toLowerCase()} ${leaving ? "leaving" : ""}`}>
      ...
      <PhotoStrip eventName={eventName} />
    </main>
  );
}
...
export function App() {
  ...
  const eventName = useMemo(getEventName, []);
  ...
  const screen = useMemo(() => {
    if (exited) return <ExitedEventScreen eventName={eventName} />;
    if (phase === "start" && startFailure) return <StartUnavailableScreen kind={startFailure} eventName={eventName} onRetry={() => setStartFailure(null)} onExit={() => setExitDialog(true)} />;
    if (phase === "start") return <StartScreen variant={variant} eventName={eventName} onStart={startCycle} onExit={() => setExitDialog(true)} />;
    if (phase === "strip") return <StripScreen variant={variant} eventName={eventName} remaining={remaining} leaving={leaving} />;
  ...
  }, [variant, phase, completed, countdown, remaining, error, leaving, exited, startFailure, eventName]);

  return (
    <div className={`prototype-stage variant-${variant}`}>
      {screen}
      {exitDialog && <ExitConfirmation eventName={eventName} onCancel={() => setExitDialog(false)} onConfirm={() => {
        setExitDialog(false);
        if (window.parent !== window) window.parent.postMessage({ type: "fotohavn-exit-event" }, "http://127.0.0.1:43173");
        else setExited(true);
      }} />}
