...
- Issue 21 supersedes the earlier printer requirement: `No Printer` is the default valid printing option, does not require a device check, and is the only path under validation here. Retain `DNP DS-RX1HS` only as historical prototype context.
- For issue 10 validation, starting an Event opens the approved issue 20 guest prototype rather than the earlier placeholder Guest Start.
