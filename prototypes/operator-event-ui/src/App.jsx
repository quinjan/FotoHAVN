...
function SetupModal({ variant, event, isNew, initialExpanded, onCancel, onSave, onStart }) {
  ...
  const [printer, setPrinter] = useState(event ? "DNP DS-RX1HS" : "");
  ...
  const basePrinter = event ? "DNP DS-RX1HS" : "";
  ...
  const validate = (intent) => {
    ...
    if (!printer) next.printer = "Choose a Printer to continue.";
    setErrors(next);
    ...
    <div className="modal-layer" role="presentation">
      <section className={`setup-modal variant-${variant}`} role="dialog" aria-modal="true" aria-labelledby="setup-title">
        <header className="modal-header">
          <div><p className="eyebrow">Event setup</p><h2 id="setup-title">{modalTitle}</h2><p>{isNew ? "Name the Event and choose the Camera and Printer for this booth run." : "Update the Event name, Camera, or Printer. Camera Tuning is available when you need it."}</p></div>
        ...
        <div className="setup-content">
          <section className="identity-grid">
            ...
            <label className={`field printer-field ${printer !== basePrinter ? "dirty" : ""} ${errors.printer ? "invalid" : ""}`}>
              ...
              <small>{errors.printer ?? "Only eligible Printers are shown."}</small>
            </label>
          </section>
...
function Preflight({ event, onDone }) {
  ...
  const items = ["Camera connected", "Printer ready", "Storage available"];
...
function GuestStart({ event, onExit }) {
  return <div className="guest-start"><header><span className="brand"><span>F</span>FotoHAVN</span><button onClick={onExit}><Power />Exit Event</button></header><main><Status>Booth ready</Status><p className="eyebrow">{event.name}</p><h1>Touch to begin</h1><p>Your photo strip is a few moments away.</p><button className="guest-button">Start</button></main></div>;
}
...
export function App() {
  ...
  return <div className="prototype-stage">
    {active ? <GuestStart event={active} onExit={() => setDialog({ kind: "exit", event: active })} /> : <Landing
    ...
    {dialog?.kind === "start" && <Confirmation title={`Start “${dialog.event.name}”?`} body="FotoHAVN will check the camera, printer, and storage before opening the guest Start screen." actionLabel="Start Event" onCancel={() => setDialog(null)} onConfirm={() => beginPreflight(dialog.event)} />}
